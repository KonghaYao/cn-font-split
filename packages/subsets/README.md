...
| --------- | ------------------ | ------ | ------- | ------ | -------- |
| ✅18.0.0+ | ✅1.30.0 ❌^1.30.0 | 114    |         |        | ❌Coming |
...
1. 1.30.0 为推荐版本，后续版本中使用了本地 npm 路径导入，导致性能衰弱。可以采用 `--no-npm` 避免。
2. 在 1.30.0 版本之后的一些特性导致了多线程失败。需要在配置中添加选项 `threads.image = false` 解决。
