...
pub fn add_remain_chars_plugin(subsets: &mut Vec<BTreeSet<u32>>, remaining_chars_set: &mut BTreeSet<u32>) {
    subsets.push(remaining_chars_set.clone());
}
pub fn auto_subset_plugin(subsets: &mut Vec<BTreeSet<u32>>, remaining_chars_set: &mut BTreeSet<u32>) {
    let new_subsets = subsets.iter().flat_map(|subset| {
        let mut count = 0;
