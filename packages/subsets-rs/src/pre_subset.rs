...
use opentype::truetype::tables::character_mapping::{Encoding, Encoding0, Encoding12, Encoding14, Encoding4, Encoding6};
...
pub fn analyze_gsub(font: &Font, font_file: &mut std::fs::File) -> Vec<Vec<u32>> {
    ...
    let mut feature_tags: Vec<&str> = data.features.headers
        .iter()
        .map(|h| h.tag.as_str().expect("Invalid tag"))
        .collect();
    ...
    let all_maybe_relative_glyph: Vec<Vec<u16>> = records.iter().flat_map(|r| {
        r.tables.iter().flat_map(|t| -> Vec<Vec<u16>>{
            match t {
                ...
                Type::LigatureSubstitution(sub) => {
                    // println!("{:?}", sub);
                    let result = sub.records.iter().flat_map(|r| {
                        r.records.iter().map(|liga|
                    ...
                    match context {
                        ChainedContext::Format1(ctx) => {
                            ...
                            let result = match &ctx.coverage {
                                ...
                                Coverage::Format2(ctx) => {
                                    ctx.records.iter().map(|r| (r.start_glyph_id..=r.end_glyph_id).collect()).collect()
                                }
                            };
                            // println!("{:?}", result);
                            result
                        ...
                        ChainedContext::Format2(ctx) => {
                            // println!("{:?}", ctx);
                            // TODO 需要测试
                            ctx.records.iter().flat_map(|r| {
                                let record = r.clone().unwrap();
                                record.records.iter().flat_map(|rr| {
                                    vec![rr.forward_indices.clone(), rr.backward_indices.clone()]
                                }).collect::<Vec<Vec<u16>>>()
                            }).collect()
                        }
                        ChainedContext::Format3(ctx) => {
                            ...
                            collect_glyph_id_from_format_1_and_2(&ctx.coverages, &mut result);
                            collect_glyph_id_from_format_1_and_2(&ctx.forward_coverages, &mut result);
                            collect_glyph_id_from_format_1_and_2(&ctx.backward_coverages, &mut result);
                            // vec![result]
                            // println!("||| {:?}\n", result);
                            vec![result]
                        }
    ...
    }).filter(|i| i.len() > 1).collect();
    // println!("{:?}", all_maybe_relative_glyph);
    all_maybe_relative_glyph.iter().map(|r| r.iter().map(|gid| cmap.get(gid).unwrap_or(&0_u32).clone()).collect::<Vec<u32>>()).collect::<Vec<Vec<u32>>>()
}

/// 将 cover 中的 glyph_id 直接注入 result
fn collect_glyph_id_from_format_1_and_2(coverages: &Vec<Coverage>, result: &mut Vec<u16>) {
    coverages.iter().for_each(|c| {
        match c {
            Coverage::Format1(cov) => {
                cov.glyph_ids.iter().for_each(|center| {
                    result.push(center.clone());
                })
            }
            Coverage::Format2(cov) => {
                cov.records.iter().for_each(|r| {
